    ...
    def lock(self):
        ...
        try:
            with transaction.atomic():
                ...
                # First try to select the calculation for update, which if it is already in a transaction in another
                # process, will raise an IntegrityError
                DbNode.objects.select_for_update(nowait=True).filter(pk=self.pk).first()

                # The first check won't catch cases where the node was already locked in the same interpreter, so for
                # that case we try an update or create, which should fail if it was already locked in the same process
