...
def try_create_secret_key():
    ...
    secret_key_full_name = os.path.join(AIIDA_CONFIG_FOLDER, SECRET_KEY_FNAME)
...
def create_base_dirs(config_dir=None):
    ...
    """
    from aiida.manage.configuration.base import AIIDA_CONFIG_FOLDER
    from aiida.manage.configuration import DAEMON_DIR, DAEMON_LOG_DIR
...
def create_config_noninteractive(profile_name='default', force_overwrite=False, **kwargs):
    ...
    if validate_email(email_v):
        profile[DEFAULT_USER_CONFIG_FIELD] = email_v
    else:
